package sedgewick.Tries;

import StdLib.StdOut;
import sedgewick.Queue.Queue;
 ...
 */
public class TrieSTAsInt  {
    private static final int R = 26;//256;        // extended ASCII
    ...
    private static class Node {
        private int val;
        private  char c = '\\'; // the value stored at this node part of the trie
   ...
   /**
    ...
    public void put(String key, int val) {
        ...
        {
            root = put(root,'\b', key, val, 0);
        }
    }

    private Node put(Node x, char c1, String key, int val, int d) {
        if (x == null)
        {
            x = new Node();
            x.c = c1;
        ...
        x.next[c-97] = put(x.next[c-97],c, key, val, d+1);
    ...
    private void collect(Node x, StringBuilder prefix, String pattern, Queue<String> results) {
        ...
        if (c == '.') {
            for (char ch = 0+97; ch < R+97; ch++) {
                prefix.append(ch);
                collect(x.next[ch-97], prefix, pattern, results);
                prefix.deleteCharAt(prefix.length() - 1);
            }
        }
        else {
            prefix.append(c);
            collect(x.next[c-97], prefix, pattern, results);
            prefix.deleteCharAt(prefix.length() - 1);
        }
    }
    ...
    private int longestPrefixOf(Node x, String query, int d, int length) {
        ...
        return longestPrefixOf(x.next[c-97], query, d+1, length);
    }
    ...
    public static void main(String[] args) {
        ...
        TrieSTAsInt  st = new TrieSTAsInt ();
        /*for (int i = 0; !StdIn.isEmpty(); i++) {
            String key = StdIn.readString();
            st.put(key, i);
        }*/
        String[] arrayIn = {"she", "sells", "sea", "shells", "by" ,"the", "sea", "shore"};
        for (int i = 0; i < arrayIn.length; i++) {
        String key = arrayIn[i];
        st.put(key, i+1);
        }
        // print results
        if (st.size() < 100) {
